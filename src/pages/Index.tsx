...
const Index = () => {
  ...
  return <div className="min-h-screen bg-background overflow-x-hidden">
      ...
      {/* Press/Media Section - Now simplified */}
      <PressMediaSection />

      {/* About Section - Add id for navigation */}
            ...
            {testimonials.map((testimonial, index) => <Card key={index} className="testimonial-card group bg-card/50 backdrop-blur-sm border-border/50 hover:border-border transition-all duration-500">
                <CardContent className="p-6 md:p-8">
                  <div className="flex mb-4 md:mb-6">
                    {[...Array(testimonial.rating)].map((_, i) => <Star key={i} className="h-4 w-4 md:h-5 md:w-5 text-blue-500 fill-current" />)}
                  </div>
      ...
      {/* Enhanced Footer - Update id since about is now a separate section */}
